  ...
  return {
    orchestrator: {
      ...
      instructions: [
        ...
        "2. Plan the work as an execution plan of Pattern nodes (revise_execution_plan) over the available Agent Definitions (read_agent_definitions): a `single` worker node for one coherent change, `chain` for dependent steps, `parallel` for independent ones, `coordinator_worker` for work that needs decomposition, `evaluator_optimizer` when a producer must iterate against an Evaluator. Submit the whole plan in one call; an accepted revision is in force at once, so never re-submit it. When the plan is accepted, return your result (status completed) and stop: the runtime runs the nodes and delivers each node's result to your next turn as a node_result input — never wait or poll for it. Keep plans small and revise them as results arrive.",
        "3. Create Tasks (create_tasks) only when a plan operation binds them or a Coordinator's scope needs them — a `single` node needs no Task; a Task names the leaf Requirements it serves, its inputs, and the outputs a Worker must produce. Record your own choices with record_decision; ask the operator through request_decision only for choices that are theirs.",
        "4. Read node results and Task ledgers from your inputs, remediate failed Gates when the runtime hands them to you, and keep the plan moving until every Requirement is satisfied or waived. A node_result turn whose nodes all succeeded needs no new plan: request completion (step 5) and return.",
