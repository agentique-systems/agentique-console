...
export class ProfileManagerService {
  ...
  create(workspaceId: string, input: { profileId?: string; sourceProfileId?: string } = {}): ManagerSession {
    ...
    const row: UserSessionRow = { id: newId("us"), workspaceId, title: input.profileId ? `Manager · ${input.profileId}` : "Manager · new profile",
      ...
      latestHandoffId: null, cumulativeCostUsd: 0, cumulativeApiDurationMs: 0, runState: "active" as const, runBaseCommit: null, pausedUntil: null, pauseReason: null, budgetUsd: null, autonomy: "standard" as const,
      // Starts on the configured default. The Manager pane renders the same
      // composer, so the operator can still move this lane's model — the
      // picker is orthogonal to the plan-approval lock beside it.
      model: null, createdAt: now, updatedAt: now };
