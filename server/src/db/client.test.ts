...
describe("database migrations", () => {
  ...
  it("adopts a pre-journal database at the baseline shape, then migrates it forward", () => {
    ...
    expect(second.sqlite.prepare("SELECT count(*) AS n FROM __drizzle_migrations").get()).toMatchObject({ n: 15 });
    ...
    expect(third.sqlite.prepare("SELECT count(*) AS n FROM __drizzle_migrations").get()).toMatchObject({ n: 15 });
    third.sqlite.close();
  });
