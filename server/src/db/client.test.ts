...
describe("database migrations", () => {
  it("applies the baseline to a fresh database and records the journal", () => {
    ...
    expect(sqlite.prepare("SELECT count(*) AS n FROM __drizzle_migrations").get()).toMatchObject({ n: 7 });
  ...
  it("adopts a pre-journal database at the baseline shape, then migrates it forward", () => {
    ...
    expect(second.sqlite.prepare("SELECT count(*) AS n FROM __drizzle_migrations").get()).toMatchObject({ n: 7 });
    ...
    expect(third.sqlite.prepare("SELECT count(*) AS n FROM __drizzle_migrations").get()).toMatchObject({ n: 7 });
    third.sqlite.close();
  });
