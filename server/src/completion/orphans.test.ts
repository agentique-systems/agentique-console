...
describe("orphaned process reaping", () => {
  it("kills a process the journal says never exited", () => {
    const { bus, repo, started } = harness();
  ...
  it("ignores a process that already exited", () => {
    const { bus, repo, started, exited } = harness();
    started("task_serve", 436589, "node", ["serve.mjs"]);
    exited("task_serve");
    const kill = vi.fn();

    expect(reapOrphanedProcesses({ repo, bus, kill, isAlive: () => true, readCmdline: () => "node\0serve.mjs\0" })).toBe(0);
    expect(kill).not.toHaveBeenCalled();
  });
  ...
  it("closes the ledger entry so the next boot does not rescan forever", () => {
    ...
    reapOrphanedProcesses({ repo, bus, kill: vi.fn(), isAlive: () => false, readCmdline: () => null });
    ...
    expect(repo.listOrphanedProcesses()).toHaveLength(0);
  });

  it("is a no-op on a clean journal", () => {
    const { bus, repo } = harness();
    expect(reapOrphanedProcesses({ repo, bus, kill: vi.fn() })).toBe(0);
  });
});
