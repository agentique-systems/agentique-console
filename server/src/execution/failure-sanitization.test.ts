 ...
 */
import { ConflictError } from "@agentique-console/core";
...
const SECRETS = [CONTENT, PATH, KEY, RAW_INPUT, "MARKER"];
...
describe("data-access failure sanitization", () => {
  it("read failures: an underlying exception carrying content, a path, a storage key, or raw input reaches neither the outcome nor the diagnostic — only the tool, ids, digest, and failure kind do", async () => {
    ...
    try {
      ...
      const cases: [string, () => void, string][] = [
        ["a storage read naming a path and content", () => vi.spyOn(h.blobs, "get").mockImplementationOnce(() => { throw Object.assign(new Error(`EACCES: permission denied, open '${PATH}' while reading ${CONTENT}`), { code: "EACCES" }); }), "read_artifact failed: Error:EACCES"],
        ["a metadata read naming a storage key", () => vi.spyOn(h.stores.artifacts, "get").mockImplementationOnce(() => { throw Object.assign(new Error(`database disk image is malformed at ${KEY}`), { name: "SqliteError", code: "SQLITE_CORRUPT" }); }), "read_artifact failed: SqliteError:SQLITE_CORRUPT"],
      ];
      ...
      expect(decisions).toEqual({ kind: "failed", tool: "read_decisions", message: "read_decisions failed: Error" });
      ...
      expect(tasks).toEqual({ kind: "failed", tool: "read_tasks", message: "read_tasks failed: ConflictError:conflict" });
  ...
  it("write failures: the canonical failure's kind is reported, a cleanup failure is reported beside it without replacing it or claiming the blob was removed, and no path, key, content, or input travels", async () => {
    ...
    try {
      ...
      vi.spyOn(h.blobs, "put").mockImplementationOnce(() => { throw Object.assign(new Error(`ENOSPC: no space left on device, write '${PATH}.tmp'`), { code: "ENOSPC" }); });
      const noSpace = await port.call(writeArtifact({ title: "t", content: CONTENT }));
      expect(noSpace).toEqual({ kind: "failed", tool: "write_artifact", message: "write_artifact failed: Error:ENOSPC" });
      expect(diagnostics.at(-1)).toEqual({ kind: "runtime_tool_call_failed", invocationId: invocation.id, attemptId: attempt.id, tool: "write_artifact", callDigest: expect.stringMatching(/^[0-9a-f]{64}$/), message: "write_artifact failed: Error:ENOSPC" });
      ...
      vi.spyOn(h.blobs, "remove").mockImplementationOnce(() => { throw Object.assign(new Error(`EPERM: operation not permitted, unlink '${PATH}'`), { code: "EPERM" }); });
      diagnostics.length = 0;
      h.diagnostics.length = 0;
      const failed = await port.call(writeArtifact({ title: "t", content: CONTENT }));
      expect(failed).toEqual({ kind: "failed", tool: "write_artifact", message: "write_artifact failed: SqliteError:SQLITE_CONSTRAINT" });
      const digest = h.stores.artifacts.listByRun(runId).find(() => false)?.digest;
      expect(digest).toBeUndefined();
      expect(h.diagnostics).toEqual([{ kind: "blob_cleanup_failed", digest: expect.stringMatching(/^[0-9a-f]{64}$/), message: "blob removal failed: Error:EPERM" }]);
      const orphan = (h.diagnostics[0] as { digest: string }).digest;
      expect(h.blobs.has(orphan)).toBe(true);
      expect(diagnostics).toEqual([{ kind: "runtime_tool_call_failed", invocationId: invocation.id, attemptId: attempt.id, tool: "write_artifact", callDigest: expect.stringMatching(/^[0-9a-f]{64}$/), message: "write_artifact failed: SqliteError:SQLITE_CONSTRAINT" }]);
