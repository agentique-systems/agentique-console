...
describe("classifyAttempt", () => {
  it("maps every completion and runtime interruption to the closed status and failure class with bounded detail", () => {
    ...
    expect(classifyAttempt({ completion: { kind: "approval_required", tool: "shell", call: "rm -rf" }, validation: invalid, runtimeInterruption: null, consumed: within, allocation })).toMatchObject({ status: "failed", failureClass: "tool_failure", detail: { tool: "shell", message: "tool shell requires operator approval: rm -rf" } });
