...
describe("InvocationPreparationService", () => {
  ...
  it("funds ordinary Invocations from the node, Task Invocations by reservation transfer, and final Invocations from the Run final reserve", async () => {
    ...
    try {
      ...
      const worker = h.preparation.prepare({ runId: s.created.run.id, planNodeId: node.id, role: "worker", purpose: "task", agentDefinitionRevisionId: s.worker.id, continuedFromInvocationId: null, taskIds: [task.id], patternPosition: null, funding: { source: "task_transfer", taskReservationId: reservation.id } });
      ...
      // A task Invocation executes exactly one ready Task, and the transfer must be that Task's reservation.
      expect(() => h.preparation.prepare({ runId: s.created.run.id, planNodeId: node.id, role: "worker", purpose: "task", agentDefinitionRevisionId: s.worker.id, continuedFromInvocationId: null, taskIds: [], patternPosition: null })).toThrow(/exactly one Task/);
      expect(() => h.preparation.prepare({ runId: s.created.run.id, planNodeId: node.id, role: "worker", purpose: "step", agentDefinitionRevisionId: s.worker.id, continuedFromInvocationId: null, taskIds: [task.id], patternPosition: null })).toThrow(/is running/);
      // Final-reserve funding: only on the root, only for the two uses, reserved directly from the Run.
      await complete(h, s.invocation);
      const synthesis = h.preparation.prepare({ runId: s.created.run.id, planNodeId: s.created.root.id, role: "orchestrator", purpose: "final_synthesis", agentDefinitionRevisionId: s.orchestrator.id, continuedFromInvocationId: s.invocation.id, taskIds: [], patternPosition: null, funding: { source: "run_final_reserve", use: "final_synthesis" } });
      ...
      expect(() => h.preparation.prepare({ runId: s.created.run.id, planNodeId: node.id, role: "evaluator", purpose: "evaluate", agentDefinitionRevisionId: s.worker.id, continuedFromInvocationId: null, taskIds: [], patternPosition: null, funding: { source: "run_final_reserve", use: "run_completion" } })).toThrow(/root Plan Node/);
      // An allocation the node cannot cover is refused atomically.
      expect(() => h.preparation.prepare({ runId: s.created.run.id, planNodeId: node.id, role: "worker", purpose: "step", agentDefinitionRevisionId: s.worker.id, continuedFromInvocationId: null, taskIds: [], patternPosition: null, allocation: { costUsd: 100, tokens: 1, attempts: 1 } })).toThrow(InsufficientCapacityError);
    } finally {
      h.close();
    }
  });
  ...
  it("keeps one active Orchestrator Invocation per Run and one active Coordinator Invocation per node, in logical sequence", async () => {
    ...
    try {
      const s = seedPlanningRuntime(h);
      const node = coordinatorNode(h, s);
      const orchestrator = { runId: s.created.run.id, planNodeId: s.created.root.id, role: "orchestrator" as const, purpose: "node_result" as const, agentDefinitionRevisionId: s.orchestrator.id, taskIds: [], patternPosition: null };
      ...
      const coordinator = { runId: s.created.run.id, planNodeId: node.id, role: "coordinator" as const, agentDefinitionRevisionId: s.worker.id, taskIds: [], patternPosition: null };
      ...
      expect(() => h.preparation.prepare({ runId: s.created.run.id, planNodeId: node.id, role: "worker", purpose: "step", agentDefinitionRevisionId: s.worker.id, continuedFromInvocationId: decompose.invocation.id, taskIds: [], patternPosition: null })).toThrow(/holds the coordinator role/);
      expect(() => h.preparation.prepare({ runId: s.created.run.id, planNodeId: node.id, role: "worker", purpose: "step", agentDefinitionRevisionId: s.worker.id, continuedFromInvocationId: s.invocation.id, taskIds: [], patternPosition: null })).toThrow(/belongs to PlanNode/);
  ...
  it("refuses a non-running node, a join, a foreign or unexecutable definition, and an Orchestrator definition off the root", () => {
    ...
    try {
      ...
      const base = { runId: s.created.run.id, role: "worker" as const, purpose: "step" as const, agentDefinitionRevisionId: s.worker.id, continuedFromInvocationId: null, taskIds: [], patternPosition: null };
      ...
      expect(() => h.preparation.prepare({ ...base, planNodeId: pending.id, agentDefinitionRevisionId: "agdr_000000000000000000000000" })).toThrow(/not executable by this Run/);
      const other = seedPlanningRuntime(h);
      expect(() => h.preparation.prepare({ ...base, planNodeId: other.created.root.id })).toThrow(/belongs to Run/);
      expect(() => h.preparation.prepare({ ...base, runId: other.created.run.id, planNodeId: other.created.root.id, role: "orchestrator", purpose: "operator_input", agentDefinitionRevisionId: s.worker.id })).toThrow(/cannot hold the Orchestrator role/);
      expect(h.provider.requests).toHaveLength(0);
