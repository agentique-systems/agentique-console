...
describe("request_completion", () => {
  ...
  it("is not callable by a Worker, a Coordinator, an Evaluator, or a final-synthesis turn, and a non-root caller is refused without a row", () => {
    ...
    try {
      ...
      expect(binding("worker", "step").tools).toEqual([]);
      expect(binding("worker", "task").tools).toEqual([]);
      expect(binding("coordinator", "decompose").tools).toEqual(["propose_tasks", "update_task"]);
      expect(binding("evaluator", "evaluate").tools).toEqual([]);
      expect(binding("orchestrator", "final_synthesis").tools).toEqual([]);
      expect(binding("orchestrator", "operator_input").tools).toEqual(["request_completion"]);
