...
describe("side-effect approval", () => {
  ...
  it("refuses at the store and the database a blocked transition without an open approval of that Invocation", () => {
    ...
    try {
      ...
      expect(() => h.database.sqlite.prepare("UPDATE invocations SET status = 'blocked', ended_at = '2026-01-01T00:00:00.000Z' WHERE id = ?").run(invocation.id)).toThrow(/invocations_blocked_has_decision/);
