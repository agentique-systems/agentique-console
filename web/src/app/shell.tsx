...
export function Shell() {
  ...
  useEffect(() => { if (previousWorkspace.current === workspace) return; previousWorkspace.current = workspace; useUiStore.setState({ activeUserSessionId: null, activeAgentSessionId: null, selectedTaskId: null, selectedTimelineItemId: null, selectedProfileId: null, activeManagerSessionId: null, draftOpen: false }); }, [workspace]);
  return <div className="flex h-screen min-w-[960px] flex-col overflow-hidden"><Topbar /><main className="min-h-0 flex-1"><Suspense fallback={<div className="flex h-full items-center justify-center"><Spinner className="size-4 text-muted-foreground" /></div>}><Routes>
