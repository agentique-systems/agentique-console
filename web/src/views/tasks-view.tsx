...
export function TasksView() {
  ...
  return <ResizableGroup orientation="horizontal" className="h-full" defaultLayout={defaultLayout} onLayoutChanged={onLayoutChanged}>
    ...
    <ResizablePanel id="graph" minSize="420px"><div className="relative h-full bg-background"><div className="absolute left-3 top-3 z-10 rounded border bg-card px-2 py-1 text-2xs text-muted-foreground">{data.data?.tasks.filter((t) => t.status !== "deleted" && inScope(t)).length ?? 0} tasks · {scope.kind}</div><ReactFlow nodes={displayedNodes} edges={edges} nodeTypes={nodeTypes} fitView onNodeClick={(_e, node) => selectTask(node.id)} nodesDraggable={false} nodesConnectable={false} elementsSelectable><Background gap={20} size={1} /><Controls showInteractive={false} /></ReactFlow></div></ResizablePanel><ResizableHandle />
