...
function TimelineCanvas() {
  ...
  useEffect(() => { if (!host.current || !query.data) return; const groups = query.data.lanes.map((lane) => ({ id: lane.id, content: lane.label, order: lane.order, ...(lane.kind === "agent_session" ? { nestedGroups: query.data!.lanes.filter((l) => l.parentId === lane.id).map((l) => l.id), showNested: true } : {}) }));
    const items = query.data.items.map((item) => ({ id: item.id, group: item.laneId, content: item.label, start: item.start, ...(item.end || item.kind === "turn" || item.kind === "tool" ? { end: item.end ?? new Date(), type: "range" } : { type: "point" }), className: `timeline-${item.kind} timeline-status-${item.status ?? "none"}`, title: item.label }));
