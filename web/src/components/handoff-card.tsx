import type { HandoffReference, HandoffSummary } from "@agentique-console/shared";
import { ArrowRightIcon, CircleAlertIcon, FileCheckIcon } from "lucide-react";
import { Badge } from "@/components/ui/badge";
import { cn } from "@/lib/utils";

export function HandoffCard({ handoff, sender, recipient, reference }: { handoff: HandoffSummary; sender?: string; recipient?: string; reference?: HandoffReference }) {
  return (
    <div className="surface-raised my-2 rounded-lg border border-border bg-card px-4 py-3 text-sm">
      <div className="mb-2 flex flex-wrap items-center gap-2">
        ...
        {reference && <Badge variant="outline">forwarded</Badge>}
      ...
      {reference && (
        <div className="mt-1 text-xs text-muted-foreground">
          {reference.forwardedBy} forwarded this for {reference.purpose.replaceAll("_", " ")}: {reference.expectedAction}
        </div>
      )}
