...
describe("decision kinds and policies", () => {
  it("are the exact closed sets", () => {
    expect(DECISION_KINDS).toEqual(["operator_choice", "orchestrator_choice", "requirement_waiver", "side_effect_approval", "signoff", "publish"]);
...
describe("resolution rules", () => {
  ...
  it("only the operator resolves a requirement_waiver, side_effect_approval, signoff, or publish", () => {
    for (const kind of ["requirement_waiver", "side_effect_approval", "signoff", "publish"] as const) {
