...
|---|---|
| `CONSOLE_DATA_DIR`, `CONSOLE_PORT`, `CONSOLE_HOST`, `CONSOLE_FS_ROOTS`, `CONSOLE_MODEL`, `CONSOLE_EFFORT`, `CONSOLE_BROWSER_MCP` | Retained. |
| `CONSOLE_MCP_DISABLED` | Retained, with its final meaning: a comma-separated list of approved MCP server names (`browser`) dropped from the catalog an Attempt may receive; an entry that names no approved server fails startup naming the variable. It is not a flag. |
| `CONSOLE_MCP_TOOL_TIMEOUT_MS` | Retained and implemented (amended 2026-09-02): the wall-clock bound on one MCP tool call of an Attempt, in milliseconds (at least 1000), handed to the Attempt's subprocess as the SDK's own `MCP_TOOL_TIMEOUT` and stated as the per-server `timeout` of every catalog server; unset leaves the SDK's default. |
| `CONSOLE_SKILLS_DIR` | Deleted with no replacement (amended 2026-09-02; this row previously read "retained"). An Attempt loads no skills, plugins, or ambient settings (`settingSources: []`, `skills: []` in the production adapter); the skills under `server/skills/` are operator-facing material the console never loads. There is no directory to configure. |
| `CONSOLE_AUTO_INIT_GIT` | Deleted with no replacement (amended 2026-09-02; this row previously read "retained"). The console never initializes a repository: a directory is adopted as it is — a repository root as a `git` Workspace, anything else as a `directory` Workspace, which runs everything but cannot be published atomically (execution-model §9.4). An operator who wants a git Workspace initializes the repository themselves. |
| `CONSOLE_IMPROVE_MODEL` | Deleted with no replacement. |
| `CONSOLE_AGENT_CONTEXT_RETIRE_TOKENS`, `CONSOLE_AGENT_IDLE_REAP_MS`, `CONSOLE_AGENT_SPAWN_TIMEOUT_MS`, `CONSOLE_MAX_RESIDENT_AGENTS`, `CONSOLE_MAX_RESIDENT_AGENTS_PER_SESSION`, `CONSOLE_PEER_NAME_PREFIX` | Deleted. → Resource Governor limits `CONSOLE_PROVIDER_MAX_CONCURRENCY`, `CONSOLE_PROCESS_MAX_ATTEMPTS`; Run Budget concurrency; context policy on the Agent Definition revision. No Attempt start timeout exists (amended 2026-09-02; this row previously promised `CONSOLE_ATTEMPT_START_TIMEOUT_MS`, which was never implemented and is not introduced): the Invocation-wide wall-clock deadline (`CONSOLE_ATTEMPT_MAX_WALL_CLOCK_MS`, execution-model §7.6) is the one time bound on an Attempt, so a provider that never starts is interrupted at that deadline, classified `interrupted`, and retried within the Invocation's allocation — never earlier. |
| `CONSOLE_AGENT_WORKTREES` | Deleted with no replacement (amended 2026-09-02; this row previously promised `CONSOLE_WORKTREES`, which was never implemented and is not introduced). Isolation is not optional (execution-model §9.1): every writing Invocation runs in its own worktree — of the git Workspace, or of the directory Workspace's shadow repository — and nothing disables it. |
